...
<article class="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
    <a href="{{ route('products.show', $product) }}">
        <img class="h-52 w-full object-cover" src="{{ $image }}" alt="{{ $product->name }}">
    </a>
    <div class="space-y-3 p-5">
        ...
        <div class="flex items-center justify-between">
            ...
            <a class="rounded-md bg-zinc-900 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700" href="{{ route('products.show', $product) }}">Details</a>
        </div>
    </div>
</article>
